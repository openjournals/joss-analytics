...
```{r rejections, class.source = 'fold-hide', fig.width = 8, fig.height = 8, message = FALSE}
ggplot(all_rejected, 
...
citations <- tryCatch({
  citecorp::oc_coci_cites(doi = papers$alternative.id) %>%
    dplyr::distinct() %>%
    dplyr::mutate(citation_info_obtained = lubridate::today())
}, error = function(e) {
  NULL
})
dim(citations)
...
if (!is.null(citations)) {
  ...
  if (is.null(citations[["citation_info_obtained"]])) {
    citations$citation_info_obtained <- as.Date(NA)
  }
}
...
max(citations$citation_info_obtained)
