...
df <- do.call(dplyr::bind_rows, lapply(software_urls[is_github], function(u) {
  ...
  contribs <- try({
    gh(paste0(gsub("(https://)?(www.)?github.com/", "/repos/", u0), "/stats/contributors"))
  })
  if (!is(info, "try-error") && length(info) > 1) {
    if (!is(contribs, "try-error")) {
      if (length(contribs) == 0) {
        ...
        repo_nbr_contribs_2ormore <- sum(vapply(contribs, function(x) x$total >= 2, NA_integer_))
...
ggplot(papers %>% 
  ...
  labs(x = "", y = "Number of published papers") + 
...
```{r languages, class.source = 'fold-hide', fig.width = 8}
...
ggplot(papers, aes(x = nbr_authors, y = repo_nbr_contribs_2ormore)) + 
  geom_point() + 
  theme_bw() + 
