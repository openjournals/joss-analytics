...
```{r language-bytes-plot, class.source = 'fold-hide'}
...
```{r nbr-reviewers, class.source = 'fold-hide', message = FALSE}
ggplot(papers %>%
         dplyr::filter(!grepl("rOpenSci|pyOpenSci", prerev_labels)) %>%
         dplyr::mutate(year = year(published.date)),
       aes(x = nbr_reviewers)) + geom_bar() + 
  facet_wrap(~ year) + theme_bw() + 
  labs(x = "Number of reviewers", y = "Number of submissions")
```
...
```{r papers-per-editor, class.source = 'fold-hide', message = FALSE, fig.width = 10, fig.height = 10}
ggplot(papers %>% 
         dplyr::mutate(year = year(published.date),
                       `r/pyOpenSci` = factor(
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) + 
  labs(x = "Editor", y = "Number of submissions")
```
...
citations <- citecorp::oc_coci_cites(doi = papers$alternative.id[1:50])

## TODO: Extract the records that are not present in the archive

tmpj <- rcrossref::cr_works(dois = citations$citing)$data %>%
