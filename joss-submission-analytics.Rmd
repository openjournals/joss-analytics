...
```{r check-software-repos, class.source = 'fold-show'}
...
df <- do.call(dplyr::bind_rows, lapply(software_urls[is_github], function(u) {
  info <- try({
    gh(gsub("(https://)?(www.)?github.com/", "/repos/", u))
  })
...
```{r acceptance-to-commit, class.source = 'fold-hide', warning = FALSE}
